...
def cut(chunk: str, mean_size: int = 9, previous_length: int = 0) -> list[str]:
    ...
    if len(chunk) >= mean_size:
        ...
        if chunk[size:] != '':
            out+= cut(chunk[size:], mean_size+1 if previous_length > len(chunk[:size]) else mean_size-1, len(chunk[:size]))
    elif chunk not in ('\n', ' ', '\t', '\r', ''):
        out.append(chunk)
...
if __name__ == "__main__":
    ...
    for i in [4, 7, 8, 9, 10, 11, 12]:

        out = chunkify_length(texte_support.replace("\n", ""), i)

        with open(f"chunked/length_dynamic/chunkify_length_{i}.txt", mode="w", encoding="utf-8") as f:
            for el in out:
                f.write(el + "\n")