...
def cut(chunk: str, mean_size: int = 9) -> list[str]:
    ...
    if len(chunk) >= mean_size and mean_size > 0:
        size = mean_size-1
        ...
        if chunk[size:] != '':
            out+= cut(chunk[size:], mean_size-1)
    elif chunk not in ('\n', ' ', '\t', '\r', ''):
        out.append(chunk)
...
if __name__ == "__main__":
    ...
    for i in [4, 9, 10, 11, 12]:
        ...
        with open(f"chunked/length_decreasing/chunkify_length_{i}.txt", mode="w", encoding="utf-8") as f:
            for el in out:
                f.write(el + "\n")